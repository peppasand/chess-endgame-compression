...
/* ---- BTM Illegal Position Filter ---- */
bool knight_attacks_sq(int nSq, int kSq) {
    int df = abs((nSq % 8) - (kSq % 8));
    int dr = abs((nSq / 8) - (kSq / 8));
    return (df == 1 && dr == 2) || (df == 2 && dr == 1);
}

bool is_btm_illegal(int wK, int bN) {
    /* BTM παρανομη θεση:
     * Ο λευκος βασιλιας ΔΕΝ μπορει να ειναι σε σαχ απο τον μαυρο ιππο.
     * (Σημαινει οτι ο μαυρος εκανε παρανομη κινηση αφηνοντας τον λευκο σε σαχ)
     *
     * ΠΡΟΣΟΧΗ: Ο πυργος που κανει σαχ στον μαυρο βασιλια ΕΙΝΑΙ ΝΟΜΙΜΟ —
     * ο μαυρος παιζει και βγαινει απο σαχ. ΔΕΝ φιλτραρεται. */
    return knight_attacks_sq(bN, wK);
}
...
int main(int argc, char *argv[]) {
    ...
    for (wK = 0; wK < 64; wK++) {
        ...
        for (wR = 0; wR < 64; wR++) {
            ...
            /* ΣΗΜΕΙΩΣΗ: ΔΕΝ εφαρμοζουμε mirroring στον πυργο.
             * Μολις ο wK ειναι canonical, εχουν εξαντληθει ολες οι
             * γεωμετρικες συμμετριες. Δευτερη συμμετρια σε αλλο κομματι
             * θα αφαιρουσε νομιμες θεσεις απο το dataset. */
            ...
            for (bK = 0; bK < 64; bK++) {
                for (bN = 0; bN < 64; bN++) {
                    ...
                    /* BTM: φιλτρο παρανομων θεσεων */
                    if (!wtm && is_btm_illegal(wK, bN)) {
                        cnt_skip++; continue;
                    }
                    ...
                    /* probe_with_board επιστρεφει ΠΑΝΤΑ απο οπτικη ΛΕΥΚΟΥ:
                     *   +1 = λευκος νικα → "white"
                     *   -1 = μαυρος νικα → "black"
                     *    0 = draw         → "draw"
                     * Ισχυει ΚΑΙ για WTM ΚΑΙ για BTM */
                    const char *res_str;
                    if (result > 0) {
                        res_str = "white"; cnt_win++;
                    } else if (result < 0) {
                        res_str = "black"; cnt_black++;
                    } else {
                        res_str = "draw";  cnt_draw++;
                    }
    ...
    int expected = wtm ? 1600000 : 1800000;  /* χωρις mirror wR — πληρες dataset */
