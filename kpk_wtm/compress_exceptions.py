...
def print_report(failures, bin_file, bin_size, csv_file, total_positions=None, cv_accuracy=None):
    """Εκτυπωνει αναφορα συμπιεσης."""

    N = len(failures)
    total = total_positions if total_positions else N  # fallback
    ...
    # Ο Θανου: .emd αρχεια (compressed Nalimov) ~7 KB για KPK
    nalimov_approx = 7 * 1024
    ...
    print(f"  Nalimov .emd (εκτ.) : {nalimov_approx:>8} bytes ({nalimov_approx/1024:.1f} KB)")
    ...
    if cv_accuracy and total_positions:
        cv_correct = total_positions - N
        print(f"  Ακριβεια J48 (training set): {cv_accuracy:.4f}% ({cv_correct}/{total_positions} σωστα)")
...
def main():
    ...
    # Φορτωνουμε το αρχικο CSV για να βρουμε τον συνολικο αριθμο θεσεων
    ...
    cv_accuracy = None
    if total_positions:
        cv_accuracy = 100.0 * (total_positions - N) / total_positions

    # Αναφορα
    print()
    print_report(failures, bin_file, bin_size, csv_file, total_positions, cv_accuracy)
