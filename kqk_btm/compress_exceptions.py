...
def load_failures(csv_file):
    ...
    Υποστηριζει KPK (wPFile/wPRank), KQK (wQFile/wQRank), KRK (wRFile/wRRank).
    Επιστρεφει λιστα απο (wKSq, wPieceSq, bKSq, actual, predicted).
    """
    failures = []
    with open(csv_file, newline='', encoding='utf-8-sig') as f:
        ...
        # Ανιχνευση στηλης κομματιου
        if 'wPFile' in headers:
            piece_file_col, piece_rank_col = 'wPFile', 'wPRank'
        elif 'wQFile' in headers:
            piece_file_col, piece_rank_col = 'wQFile', 'wQRank'
        elif 'wRFile' in headers:
            piece_file_col, piece_rank_col = 'wRFile', 'wRRank'
        else:
            # Fallback: ψαξε οποιο File/Rank υπαρχει εκτος απο wK και bK
            piece_file_col = next((h for h in headers
                                   if h.endswith('File') and h not in ('wKFile','bKFile')), None)
            piece_rank_col = next((h for h in headers
                                   if h.endswith('Rank') and h not in ('wKRank','bKRank')), None)
            if not piece_file_col:
                raise ValueError(f"Δεν βρεθηκε στηλη κομματιου στο {csv_file}")

        for row in reader:
            wk_file = int(row['wKFile'])
            wk_rank = int(row['wKRank'])
            wp_file = int(row[piece_file_col])
            wp_rank = int(row[piece_rank_col])
...
def print_report(failures, bin_file, bin_size, csv_file, total_positions=None, train_accuracy=None):
    ...
    if 'kqk' in fname:
        nalimov_approx = 5961    # kqk_nbb.emd
        endgame = 'KQK'
    elif 'krk' in fname:
        nalimov_approx = 7609    # krk_nbb.emd (εκτιμηση)
        endgame = 'KRK'
    else:
        nalimov_approx = 16589   # kpk_nbb.emd
        endgame = 'KPK'
