...
Χρηση:
    python analyze.py <csv> <predictions.arff>

Παραδειγματα:
    python analyze.py csv-arff\\kpk_wtm_v7.csv predictions.arff
    python analyze.py csv-arff\\kqk_btm_v1.csv predictions_kqk.arff

Σημειωση:
    Το validation (πριν Weka) γινεται αυτοματα στο add_attributes.py.
    Υποστηριζει KPK, KQK, KRK — ανιχνευει αυτοματα το κομματι.

Εξοδος:
    *_evaluation.txt  — αναφορα αποτελεσματων
    *_failures.csv    — λανθασμενες θεσεις για Meta-Mining
...
def get_attr_cols(rows):
    return [c for c in rows[0].keys() if c != 'result']
...
def symmetry_analysis(rows):
    ...
    for attr in nominal:
        ...
        # Ελεγχος αν εχουμε wPFile στα columns (KPK μονο)
        # Για KQK/KRK δεν υπαρχει πιονι — παραλειπεται η symmetry analysis
        piece_file_col = None
        if 'wPFile' in rows[0]:
            piece_file_col = 'wPFile'
            piece_label = 'πιονιου'
        elif 'wQFile' in rows[0]:
            piece_file_col = 'wQFile'
            piece_label = 'βασιλισσας'
        elif 'wRFile' in rows[0]:
            piece_file_col = 'wRFile'
            piece_label = 'πυργου'

        if piece_file_col is None:
            continue

        lines.append(f"\n{attr}=Y λαθη ανα {piece_file_col} (στηλη {piece_label}):")
        by_file = Counter(r[piece_file_col] for r in errors)
        total_e = len(errors)
...
def evaluation_stats(rows, preds, csv_filename):
    ...
    if preds and 'predicted' in preds[0] and 'result' in preds[0]:
        ...
        # CSV format: inst, actual, predicted, error, ...
...
def export_failures(combined, csv_filename):
    """
    Εξαγει τις λανθασμενες θεσεις σε CSV για Meta-Mining στο Weka.
    """
    failures = [c for c in combined if c['actual'] != c['predicted']]
    if not failures:
        return None
    ...
    fieldnames = list(failures[0]['row'].keys()) + ['predicted']
...
def update_history(csv_filename, accuracy, wrong, attr_count, leaf_count=None):
    ...
    history_file = os.path.join(base_dir, 'results_history.csv')
...
def information_gain_analysis(rows, combined):
    ...
    def gain_ratio(rows, attr):
        ...
        total   = len(rows)
        h_total = entropy(rows)
        # Ομαδοποιηση ανα τιμη attribute
        groups = {}
        for r in rows:
            v = r.get(attr, '')
            groups.setdefault(v, []).append(r)
