...
def add_attributes(raw_csv, version, wtm=True):
    ...
    with open(raw_csv, newline='', encoding='utf-8-sig') as f_in:
        ...
        for row in reader:
            # BTM: το probe γραφει 'white'/'draw' (μετα τη διορθωση του probe_kpk_btm.cpp)
            # Καμια αλλαγη απαιτειται — το CSV ειναι ηδη κανονικοποιημενο
            rows.append(row)
    ...
    _run_validation(computed_rows, out_file, attrs, wtm)

    return out_file


def _run_validation(computed_rows, out_file, attrs, wtm=True):
    """
    ...
    # Στο BTM παραλειπεται ο ελεγχος 100% Win —
    # wPAheadInTheRace=Y δεν σημαινει παντα Win οταν κινειται ο μαυρος
    if not wtm:
        win_pct = 100*wins/total
        print("  [INFO] BTM mode: παραλειπεται ελεγχος 100% Win")
        print(f"  [INFO] Win%={win_pct:.1f}% (αναμενομενο ~58% για BTM)")
        if 55.0 <= win_pct <= 62.0:
            print(f"  [OK]   Win% εντος αναμενομενου ευρους (55-62%)")
        else:
            print(f"  [WARN] Win% εκτος αναμενομενου ευρους (55-62%)")
            problems += 1
        print()
        if problems == 0:
            print("Validation: OK — BTM dataset ειναι ορθο.")
            print(f"Επομενο βημα: python csv_to_arff.py {out_file} btm")
        else:
            print(f"Validation: {problems} προβληματικα.")
        return

    # Ελεγχος nominal attributes για WTM: Y αλλα result=draw
